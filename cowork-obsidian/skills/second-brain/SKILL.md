---
name: second-brain
description: Operate on a local Obsidian vault as the user's second brain. Triggers on "/second-brain", "build my wiki", "update my wiki", "second brain health check", "fold my outputs back", "what's in my second brain about X", or any request to read, build, or maintain a wiki from raw notes in a configured Obsidian vault. v0.2.0 — mode-aware (external / overlay / scaffold). Reads `projects/second-brain/vaults.md` to find configured vault paths and modes. Three canonical commands — `build`, `update`, `health-check` — plus passive read across all wikis for context retrieval. When invoked with no command, offers a use-case picker tailored to the user's persona.
---
